...
public:
	...
	double getLastDelta() { return m_last_delta; }
	double getLastIter() { return m_last_iter; }
	...
	double fitTest(double* T, const int32_t T_num, Matrix& R, Matrix& t, double indist);

  
private:
	// iterative fitting
	void fitIterate(double *T,const int32_t T_num,Matrix &R,Matrix &t, double indist = -1);
	void fitIterateTest(double* T, const int32_t T_num, Matrix& R, Matrix& t, double indist);
...
protected:
	...
	int32_t m_last_iter;  // max number of iterations
	double  m_last_delta; // min parameter delta

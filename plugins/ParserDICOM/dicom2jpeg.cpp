...
Imebra 4.3.0.1 changeset a43d535aa731
...
int convert( std::string srcFname, std::string dstFname )
{
	...
	try
	{
		...
		int ffmpegFlag(-1);// (findArgument("-ffmpeg", argc, argv));
		size_t framesCount(0);

		std::unique_ptr<DataSet> loadedDataSet(CodecFactory::load( srcFname.c_str(), 2048 ));

		outputDatasetTags(*loadedDataSet, L"");

		// Separate the extension from the file name
		std::string outputFileName(dstFname);
		std::string extension;
		size_t dotPos(outputFileName.rfind('.'));
		if (dotPos != outputFileName.npos)
		{
		...
		try
		{
			...
			for (std::uint32_t frameNumber(0); ; ++frameNumber)
			{
				...
				//{
					jpegFileName << "_";
					if (frameNumber < 100) jpegFileName << "0";
					if (frameNumber < 10) jpegFileName << "0";
					jpegFileName << frameNumber;
				//}
				jpegFileName << extension;
...
//			}
...
#if defined(WIN32) || defined(WIN64)
			//return (int)_spawnvp(_P_WAIT, argv[ffmpegFlag + 1], ffArgv.get());
			return (int)_spawnvp(_P_WAIT, "dpVision", ffArgv.get());
#else
			char *environment[] = { 0 };
	...
	catch (const std::exception& e)
	{
		...
		convertMessage.setString(TagId(10, 10), imebra::ExceptionsManager::getExceptionTrace(), imebra::tagVR_t::LT);
		//std::wcout << std::endl << "Error: " << std::endl << convertMessage.getUnicodeString(TagId(10, 10), 0) << std::endl;
		UI::MESSAGEBOX::error(convertMessage.getUnicodeString(TagId(10, 10), 0));
		return 0;
	}
}

...
{
	setDescr("Scene definition text files");
	m_exts.insert("atmdl");
...
QString CParserATMDL::parseType_string(QTextStream& in)
{
	...
	if (slowo.front() == "\"")
	{
		...
		while (slowo.back() != "\"")
		{
			...
			if ((znak == '\n') || (znak == '\r'))
			{
				qInfo() << QString::fromUtf8("UWAGA: Napotkano koniec linii bez zamkni�cia cudzys�owu") << Qt::endl;
				break;
			}
			slowo += znak;
		}
...
QList<double> CParserATMDL::parseType_MatrixOfDouble(QTextStream& in, int size)
{
	...
	{ 	//qInfo() << "\033[31m" << "To jest komunikat b��du" << "\033[0m" << Qt::endl;
		...
		qWarning() << "\033[33mOczekiwano macierzy o rozmiarze " + QString::number(size) + " element�w. Odczytano " + QString::number(qCoords.size()) + " element�w.\033[0m" << Qt::endl;
	}
...
CTransform CParserATMDL::parseProperty_rotation(QTextStream& in)
{
	...
	while (slowo != "}")
	{
		...
		{
			qInfo() << QString::fromUtf8("B��D. Nierozpoznany symbol ") << slowo << Qt::endl;
			return CTransform();
		}
...
CMovement::FrameVal CParserATMDL::parseProperty_frame(QTextStream& in)
{
	...
	while (slowo != "}")
	{
		...
		{
			qInfo() << QString::fromUtf8("Poprawnie odczytano klatk� animacji") << Qt::endl;
		}
		else
		{
			qInfo() << QString::fromUtf8("B��D. Nierozpoznany symbol ") << slowo << Qt::endl;
...
CModel3D* CParserATMDL::load(const QString path, bool wait)
{
	...
	{
		UI::STATUSBAR::setText("Can'partialTransformation open file.");
		return nullptr;
	}
	...
	while (!in.atEnd())
	{
		...
		{
			qCritical() << "\033[33m" << QString::fromUtf8("B��D. Nierozpoznany symbol ") << slowo << "\033[0m" << Qt::endl;
			break;
		}
	}
...
QString CParserATMDL::obj2atmdl(CBaseObject* obj)
{
	QString text("");
	...
	switch (obj->type())
	{
		case CObject::MODEL:
			...
			text += "  matrix " + ((CModel3D*)obj)->transform().toString("[", "]", ",") + "\n";
			break;
		case CObject::MESH:
		case CObject::CLOUD:
			text += "shell {\n";
		...
		case CObject::MOVEMENT:
			...
			{
				text += "    frame { delay " + QString::number(double(fv.msec) / 1000) + " matrix " + fv.t.toString("[","]",",") + " }\n";
			}
			text += "  }\n";
			break;
	}
...
{
	QString text("# ATMDL File generated by dpVision\n\n");
...
bool CParserATMDL::save(CModel3D* obj, const QString path)
{
	...
	{
		UI::STATUSBAR::setText("Can't create .amtl file.");
		return false;
	}

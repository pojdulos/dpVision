...
{
	UI::PLUGINPANEL::create(m_ID, L"Test ICP");

	UI::PLUGINPANEL::addLabel(m_ID, L"label01", L"Min delta:", 29, 0);
	UI::PLUGINPANEL::addEditBox(m_ID, L"editBoxDelta", L"Min delta", L"0.0001", 29, 1);
	UI::PLUGINPANEL::addLabel(m_ID, L"label02", L"Max iter:", 30, 0);
	UI::PLUGINPANEL::addEditBox(m_ID, L"editBoxIter", L"Max iter", L"500", 30, 1);
	//UI::PLUGINPANEL::addLabel(m_ID, L"label02a", L"K¹t:", 31, 0);
	//UI::PLUGINPANEL::addEditBox(m_ID, L"editBoxKat", L"K¹t", L"90", 31, 1);
	...
	//UI::PLUGINPANEL::addButton(m_ID, L"dist_calc", L"Odleg³oœci A z B", 32, 0, 1, 2);

	//UI::PLUGINPANEL::addButton(m_ID, L"testICP", L"Test ICP", 34, 0, 1, 2);

	UI::PLUGINPANEL::addLabel(m_ID, L"label03", L"Last delta:", 35, 0);
	UI::PLUGINPANEL::addLabel(m_ID, L"label04", L"???", 35, 1);
	UI::PLUGINPANEL::addLabel(m_ID, L"label05", L"Last iter:", 36, 0);
	UI::PLUGINPANEL::addLabel(m_ID, L"label06", L"???", 36, 1);
	UI::PLUGINPANEL::addLabel(m_ID, L"label07", L"Best fit:", 37, 0);
	UI::PLUGINPANEL::addLabel(m_ID, L"label08", L"???", 37, 1);

	//UI::PLUGINPANEL::addButton(m_ID, L"rot1", L"testRot 1", 40, 0, 1, 2);
	//UI::PLUGINPANEL::addButton(m_ID, L"rot2", L"testRot 2", 41, 0, 1, 2);

	UI::PLUGINPANEL::addButton(m_ID, L"fit", L"Only Fit", 45, 0, 1, 2);
...
bool ConcretePlugin::onMousePick(Plugin::PickEvent pickEvent)
{
	...
	return true; // 'true' oznacza, ¿e obs³u¿ono komunikat i program ma ju¿ nie obs³ugiwaæ tego klikniêcia
}
...
void ConcretePlugin::onButton(std::wstring name)
{
	...
	else if (0 == name.compare(L"fit"))
	{
		...
		if (sel.size() > 1)
		{
			...
			// pierwszy zaznaczony
			CModel3D* objRuchomy = AP::WORKSPACE::getModel(*isel);
			isel++;

			// drugi zaznaczony
			CModel3D* objNieruchomy = AP::WORKSPACE::getModel(*isel);
			...
			CPointCloud* chmuraRuchoma = CPointCloud::findNClosest(chmuraRuchomaOrg, chmuraNieruchoma, 10000);

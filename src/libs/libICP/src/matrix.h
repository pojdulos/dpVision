...
typedef double MyFLOAT;      // double precision
//typedef float  MyFLOAT;    // single precision
...
public:
  ...
  Matrix (const int32_t m,const int32_t n,const MyFLOAT* val_); // init mxn matrix with values from array 'val'
  ...
  void getData(MyFLOAT* val_,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);
  ...
  void setVal(MyFLOAT s,int32_t i1=0,int32_t j1=0,int32_t i2=-1,int32_t j2=-1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(MyFLOAT s,int32_t i1=0,int32_t i2=-1);
  ...
  static Matrix rotMatX(const MyFLOAT &angle);
  static Matrix rotMatY(const MyFLOAT &angle);
  static Matrix rotMatZ(const MyFLOAT &angle);
  ...
  Matrix  operator* (const MyFLOAT &s);  // multiply with scalar
  Matrix  operator/ (const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix  operator/ (const MyFLOAT &s);  // divide by scalar
  Matrix  operator- ();                // negative matrix
  Matrix  operator~ ();                // transpose
  MyFLOAT   l2norm ();                   // euclidean norm (vectors) / frobenius norm (matrices)
  MyFLOAT   mean ();                     // mean of all elements in matrix
  ...
  MyFLOAT  det ();                                             // returns determinant of matrix
  bool   solve (const Matrix &M,MyFLOAT eps=1e-20);            // solve linear system M*x=B, replaces *this and M
  bool   lu(int32_t *idx, MyFLOAT &d, MyFLOAT eps=1e-20);        // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory ();
  inline MyFLOAT pythag(MyFLOAT a,MyFLOAT b);

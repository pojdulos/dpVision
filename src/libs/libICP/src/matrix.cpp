...
Matrix::Matrix (const int32_t m_,const int32_t n_,const MyFLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(MyFLOAT));
}
...
void Matrix::getData(MyFLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(MyFLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(MyFLOAT s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const MyFLOAT &angle) {
  MyFLOAT s = sin(angle);
  MyFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const MyFLOAT &angle) {
  MyFLOAT s = sin(angle);
  MyFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const MyFLOAT &angle) {
  MyFLOAT s = sin(angle);
  MyFLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const MyFLOAT &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
bool Matrix::solve (const Matrix &M, MyFLOAT eps) {
...
bool Matrix::lu(int32_t *idx, MyFLOAT &d, MyFLOAT eps) {
  ...
  int32_t i,imax,j,k;
  MyFLOAT   big,dum,sum,temp;
  MyFLOAT* vv = (MyFLOAT*)malloc(n*sizeof(MyFLOAT)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  MyFLOAT* w   = (MyFLOAT*)malloc(n*sizeof(MyFLOAT));
  MyFLOAT* rv1 = (MyFLOAT*)malloc(n*sizeof(MyFLOAT));

  int32_t flag,i,its,j,jj,k,l,nm;
  MyFLOAT   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((MyFLOAT)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((MyFLOAT)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((MyFLOAT)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  MyFLOAT   sw;
  MyFLOAT* su = (MyFLOAT*)malloc(m*sizeof(MyFLOAT));
  MyFLOAT* sv = (MyFLOAT*)malloc(n*sizeof(MyFLOAT));
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (MyFLOAT**)malloc(m*sizeof(MyFLOAT*));
  val[0] = (MyFLOAT*)calloc(m*n,sizeof(MyFLOAT));
...
MyFLOAT Matrix::pythag(MyFLOAT a,MyFLOAT b) {
  MyFLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)

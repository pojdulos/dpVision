...
public:
	...
	long read_files(std::shared_ptr<Volumetric> volum, int nbOfFiles, std::string dirPath, std::string shortName, std::string firstNumber, std::string suffix);
	long read_frames(std::shared_ptr<Volumetric> volum, imebra::DataSet& dataSet, int nbOfFrames);
	...
	bool _export(std::shared_ptr<Volumetric> volum, QString dir_name="", QString fname_base="slice");

	virtual bool save(std::shared_ptr<CModel3D> obj, const QString path);
	virtual bool save(QVector<std::shared_ptr<CBaseObject>> objects, const QString path);

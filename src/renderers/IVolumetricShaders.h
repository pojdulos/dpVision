...
layout (location = 0) in uint aCol;
...
void main()
{
    float r = float((aCol >> 24) & 0xFF) / 255.0;
    float g = float((aCol >> 16) & 0xFF) / 255.0;
    float b = float((aCol >> 8) & 0xFF) / 255.0;
    float a = float(aCol & 0xFF) / 255.0;
...
void main() {
    if (gs_in[0].isValid) {
        ...
        // Wierzcho�ki tworz�ce �ciany kostki
        vec4 vertices[8];

        // Obliczenia pozycji wierzcho�k�w
        vec3 halfSize = 0.5 * gs_in[0].vScale; // Po�owa d�ugo�ci boku kostki
		...
		vertices[2] = pointPos + vec4(-halfSize[0],  halfSize[1], -halfSize[2], 0.0); // Lewy g�rny tylny
		vertices[3] = pointPos + vec4( halfSize[0],  halfSize[1], -halfSize[2], 0.0); // Prawy g�rny tylny
		...
		vertices[6] = pointPos + vec4(-halfSize[0],  halfSize[1],  halfSize[2], 0.0); // Lewy g�rny przedni
		vertices[7] = pointPos + vec4( halfSize[0],  halfSize[1],  halfSize[2], 0.0); // Prawy g�rny przedni
...
in vec3 vertexColor;
in vec3 FragPos; // Pozycja fragmentu/prymitywu w przestrzeni �wiata
...
void main()
{
    ...
    vec3 lightPos = viewPos;  // Pozycja �wiat�a od strony ekranu
    //vec3 lightPos = vec3(0, 100, 600);  // Pozycja �wiat�a
    vec3 lightColor = vec3(1.0, 1.0, 1.0);  // Kolor �wiat�a
    float ambientStrength = 0.6;
    float specularStrength = 0.5;  // Si�a �wiat�a spekularnego
    float shininess = 32.0;  // Po�ysk (shininess)

    // Obliczenia o�wietlenia (tutaj mo�esz doda� w�asn� logik�)
    vec3 norm = vec3(0.0,0.0,1.0); //kierunek "do ekranu"
    vec3 lightDir = normalize(lightPos - FragPos);  // Poprawione obliczanie kierunku �wiat�a

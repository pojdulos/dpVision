#include "Animation.h"

#include "Annotation.h"
#include "AnnotationPoint.h"

#include "PMFactory.h"
#include "Utilities.h"

#include "AP.h"

#include "MainApplication.h"

#include "GLViewer.h"
#include "Parser.h"


CAnimation::CAnimation() : CObject(nullptr)
{
	setLabel("animation");

	m_bOK = true;

	//bAnime = (AP::mainApp().globalParams.RunAnimation != 0);
	//iAnimeDir = (AP::mainApp().globalParams.RunAnimation < 0) ? -1 : 1;
	//lastanim = 0;

	m_annotations.clear();

	bDrawBB = true;

	//m_fastmeshData.iMeshType = CMesh::MESHTYPE_NONE;
	//m_fastmeshData.setParent( this );
}

CAnimation::CAnimation( const CAnimation &m ) : CObject(m)
{
	setLabel("copy_of_" + m.m_label);

	m_bOK = true;

	bDrawBB = true;

	//bAnime = (AP::mainApp().globalParams.RunAnimation != 0);
	//iAnimeDir = (AP::mainApp().globalParams.RunAnimation < 0) ? -1 : 1;
	//lastanim = 0;

	m_transform = m.m_transform;

	plikSiatki.setFile(m.plikSiatki.absoluteFilePath());

	//m_annotations = CAnimation::Annotations( m.m_annotations );

	m_transform.setOrigin( (m_max + m_min) / 2 );

	//if ( ((CObject*)getChild())->type() == CObject::MESH )
	//{
	//	createFastMesh();
	//}
}


CAnimation::~CAnimation()
{
	// Nie jestem pewien czy tu nie ma wycieku pami�ci,
	// ale pr�ba usuwania poszczeg�lnych annotacji za pomoc� delete
	// ko�czy sie wyj�tkiem przy zamykaniu programu
	// 
	// temat do zbadania!!!

	m_annotations.clear();
}


//void CAnimation::PMeshEcoll( GLuint rzm, bool checkPoints )
//{
//	if ( getChild()->type() != CObject::MESH ) return;
//
//	CMesh &d = *((CMesh *)getChild());
//
//	if ( ( rzm < 0 ) || ( rzm >= d.faces().size() ) ) return;
//
//	this->setSelfVisibility(false);
//	this->setKidsVisibility(false);
//
//	//this->deleteFastMesh();
//
//	PMFactory pmf;
//
//	if ( checkPoints )
//	{
//		for ( CAnimation::Annotations::iterator it = m_annotations.begin(); it != m_annotations.end(); it++ )
//		{
//			if ( it->second->type() == CAnnotation::POINT )
//			{
//				pmf.mSolidPoints.insert( ((CAnnotationPoint*)it->second)->getPoint());
//			}
//		}
//	}
//
//	pmf.eColl( d, rzm );
//
//	m_min = ((CMesh*)getChild())->getMin();
//	m_max = ((CMesh*)getChild())->getMax();
//
//	CPoint3f delta = m_max - m_min;
//
////AT	m_transform.setOrigin( (m_max + m_min ) / 2 );
////AT	m_transform.setScale( 100.0f/MAX3( delta.X(), delta.Y(), delta.Z() ) );
//
//	//d.iMeshType &= 0xFFF0;
//	//d.iMeshType |= CMesh::MESHTYPE_PROGRESSIVEMESH3V;
//
//	d.renderFacesAsEdges(false);
//	d.renderFacesAsPoints(false);
//	d.renderSmoothVertex(false);
//	d.renderWithTexture(d.hasTexture());
//
//	//this->createFastMesh();
//	this->setSelfVisibility(true);
//	this->setKidsVisibility(true);
//}
//
//void CAnimation::PMeshVsplit( GLuint rzm )
//{
//	if ( getChild()->type() != CObject::MESH ) return;
//
//	CMesh &d = *((CMesh *)getChild());
//
//	PMFactory pmf;
//
//	//CFileInfo fi;
//	//
//	//fi.SetFullPath( plikSiatki.absoluteFilePathW().c_str() );
//	//fi.SetExt( "out" );
//
//	pmf.vSplit( d, rzm );
//
//	m_min = d.getMin();
//	m_max = d.getMax();
//
//	CPoint3f delta = m_max - m_min;
//
//	m_transform.setOrigin((m_max + m_min) / 2);
//	m_transform.setScale(100.0f / MAX3(delta.X(), delta.Y(), delta.Z()));
//
//	//d.iMeshType &= 0xFFF0;
//	//d.iMeshType |= CMesh::MESHTYPE_PROGRESSIVEMESH3V;
//
//	UI::STATUSBAR::printf( "I'm returning from CAnimation::PMeshVsplitSelf()..." );
//}

//void CAnimation::deleteFastMesh()
//{
//	m_fastmeshData.vertices().clear();
//	//m_fastmeshData.iMeshType = CMesh::MESHTYPE_NONE;
//}
//
//void CAnimation::createFastMesh()
//{
//	if ( getChild()->type() != CObject::MESH ) return;
//
//	CMesh &d = *((CMesh *)getChild());
//
//	CMesh::Vertices::iterator it;
//	GLuint orgsize = d.vertices().size();
//
//	m_fastmeshData.vertices().clear();
//	//m_fastmeshData.iMeshType = CMesh::MESHTYPE_NONE;
//
//	if ( orgsize > 50000 )
//	{
//		GLuint newsize = 10000;
//		float delta = (float)orgsize / newsize;
//
//		try {
//			m_fastmeshData.vertices().resize( newsize );
//		}
//		catch ( ... ) {
//			return;
//		}
//
//		float j;
//		for ( unsigned int i=0; i<newsize; i++ )
//		{
//			j = delta * i;
//
//			m_fastmeshData.vertices()[ i ] = d.vertices()[ (GLuint)j ];
//		}
//
//		//m_fastmeshData.iMeshType = CMesh::MESHTYPE_VERTEXCLOUD;
//	}
//}


void CAnimation::importChildrenGeometry()
{
	resetBoundingBox();
	for (auto child : m_data)
	{
		if (child.second->category() == CBaseObject::Category::OBJECT)
		{
			CObject* obj = (CObject*)child.second;
			expand(*obj);
		}
	}

//	m_min = ((CObject*)getChild())->getMin();
//	m_max = ((CObject*)getChild())->getMax();

//	CPoint3f delta = m_max - m_min;
	CPoint3f ctr = (m_max + m_min) / 2;
	
	m_transform.setOrigin( ctr );
}


//void CAnimation::setOrigin(CTransform::Origin t)
//{
//	switch (t)
//	{
//	case CTransform::Origin::BBOX_CTR:
//		m_transform.setOrigin( (m_max + m_min) / 2 );
//		break;
//	case CTransform::Origin::WEIGHT_CTR:
//		if ((getChild()->type() == CObject::MESH) || (getChild()->type() == CObject::CLOUD) || (getChild()->type() == CObject::ORDEREDCLOUD))
//		{
//			m_transform.setOrigin(((CPointCloud*)getChild())->getCenterOfWeight());
//		}
//		break;
//	case CTransform::Origin::ZERO:
//		m_transform.setOrigin( CPoint3d(0.0, 0.0, 0.0) );
//		break;
//	}
//}


void CAnimation::applyTransform(CTransform to)
{
	//set the new origin to the same point as before
	CPoint3d c = m_transform.origin() + m_transform.translation() - to.translation();
	to.setOrigin(c);

	// transform all children
	CObject::applyTransformation(m_transform, to);

	//transform all annotations
	for (auto &a : m_annotations)
	{
		a.second->applyTransformation(m_transform, to);
	}
	
	//CAnimation::Annotations::iterator it;
	//for (it = m_annotations.begin(); it != m_annotations.end(); it++)
	//{
	//	(*it).second->applyTransform(m_transform, to);
	//}

	importChildrenGeometry();

	m_transform.translation() = to.translation();
	m_transform.rotation() = to.rotation();
	m_transform.scale() = to.scale();
	m_transform.origin() = to.origin();
}


//CBaseObject* CAnimation::getSomethingWithId(int id)
//{
//	if (m_Id == id) return this;
//	else if (nullptr != getChild(id)) return getChild(id);
//	else if (nullptr != annotation(id)) return (CBaseObject*)annotation(id);
//	else return nullptr;
//}

//void CAnimation::prepare()
//{
//	for (auto& c:m_data) c.second->prepare();
//}


void CAnimation::renderTransform()
{
	if (m_showSelf) m_transform.render();
}

void CAnimation::renderSelf()
{
	if (bDrawBB) renderBoundingBox();
	if (bDrawBB) renderAxes();

	//if (m_visible != Visibility::HIDE_ALL)
	//{
	//	renderKids();
	//}
}


bool CAnimation::switchOption( CAnimation::Opt iOpt, CAnimation::Switch iSet )
{
	bool result = false;

	CMesh *d = (CMesh*)getChild();

	switch ( iOpt )
	{
//		case OPTION_RENDER_SMOOTH_VERTEX:
		case CAnimation::Opt::optSmoothVertices:
			if ( iSet == CAnimation::Switch::switchOn )
				result = d->renderSmoothVertex( d->hasVertexNormals() );
			else if ( iSet == CAnimation::Switch::switchOff )
				result = d->renderSmoothVertex( false );
			else
				result = d->renderSmoothVertex( d->hasVertexNormals() && !d->renderSmoothVertex() );
	
			prepare();
			break;

//		case OPTION_RENDER_FACES_AS_POINTS:
		case CAnimation::Opt::optRenderAsPoints:
			if ( getChild()->type() == CObject::LIDAR )
			{
				getChild()->switchOpt( 0, 0 );
				importChildrenGeometry();
			}
			else 
			{
				if ( iSet == CAnimation::Switch::switchOn )
					result = d->renderFacesAsPoints( d->hasFaces() );
				else if ( iSet == CAnimation::Switch::switchOff )
					result = d->renderFacesAsPoints( false );
				else
					result = d->renderFacesAsPoints( d->hasFaces() && !d->renderFacesAsPoints() );
			}
			break;

//		case OPTION_RENDER_FACES_AS_EDGES:
		case CAnimation::Opt::optRenderAsEdges:
			if ( iSet == CAnimation::Switch::switchOn )
				result = d->renderFacesAsEdges(d->hasFaces());
			else if ( iSet == CAnimation::Switch::switchOff )
				result = d->renderFacesAsEdges(false);
			else
				result = d->renderFacesAsEdges( d->hasFaces() && !d->renderFacesAsEdges());

			break;

//		case OPTION_RENDER_WITH_TEXTURE:
		case CAnimation::Opt::optRenderWithTexture:
			if ( iSet == CAnimation::Switch::switchOn )
				result = d->renderWithTexture( d->hasTexture() );
			else if ( iSet == CAnimation::Switch::switchOff )
				result = d->renderWithTexture( false );
			else
				result = d->renderWithTexture(d->hasTexture() && !d->renderWithTexture());
			
			prepare();
			break;

		default:
			break;
	}
	return result;
}



bool CAnimation::testOption( CAnimation::Opt iOption )
{
	bool result = false;

	CMesh *d = (CMesh*)getChild();

	if (d == nullptr) return false;

	switch ( iOption )
	{
//		case OPTION_RENDER_SMOOTH_VERTEX:
		case CAnimation::Opt::optSmoothVertices:
			result = d->renderSmoothVertex();
			break;

//		case OPTION_RENDER_FACES_AS_POINTS:
		case CAnimation::Opt::optRenderAsPoints:
			result = d->renderFacesAsPoints();
			break;

//		case OPTION_RENDER_FACES_AS_EDGES:
		case CAnimation::Opt::optRenderAsEdges:
			result = d->renderFacesAsEdges();
			break;

//		case OPTION_RENDER_WITH_TEXTURE:
		case CAnimation::Opt::optRenderWithTexture:
			result = d->renderWithTexture();
			break;

		default:
			break;
	}
	return result;
}


std::wstring CAnimation::infoRow()
{
	size_t n = m_data.size();
	size_t np = m_annotations.size();
	std::wstring ret = L"Model3D (id:"+ std::to_wstring(m_Id) + L"). Has ";
	
	if ( n+np == 0 )
		ret += L"not any children";
	if (n > 0)
	{
		ret += std::to_wstring(n) + ((n < 2) ? L" data child " : L" data children ");
		if (np > 0) ret += L" and ";
	}
	
	if (np > 0)
	{
		ret += std::to_wstring(np) + ((np < 2) ? L" annotation " : L" annotations ");
	}
	
	return ret;
}

//inline int CAnimation::setId(int i)
//{
//	CBaseObject::setId(i);
//
//	CBaseObject::Children newChildren;
//
//	for (CBaseObject::Children::iterator it = m_data.begin(); it != m_data.end(); it++)
//	{
//		int newId = getNewId();
//		it->second->setId(newId);
//		newChildren[newId] = it->second;
//	}
//
//	m_data = newChildren;
//
//	CAnimation::Annotations newAnnot;
//	for (CAnimation::Annotations::iterator it = m_annotations.begin(); it != m_annotations.end(); it++)
//	{
//		int newId = getNewId();
//		it->second->setId(newId);
//		newAnnot[newId] = it->second;
//	}
//
//	m_annotations = newAnnot;
//
//	return 0;
//}


void CAnimation::info(std::wstring i[4])
{
	i[0] = m_label.toStdWString();
	i[1] = std::to_wstring( m_Id );
	i[2] = L"Model 3D";
	i[3] = infoRow();
}



//void CAnimation::calcVN()
//{
//	if ( getChild()->type() != CObject::MESH ) return;
//
//	((CMesh *)getChild())->calcVN();
//}

//unsigned int CAnimation::getNewId()
//{
//	unsigned int newId = this->m_Id + 1;
//
//	while ((m_annotations.find(newId) != m_annotations.end()) || (m_data.find(newId) != m_data.end())) newId++;
//
//	return newId;
//}

//int CAnimation::addAnnotation(CAnnotation * ad)
//{
//	//unsigned int newId = getNewId();
//	//ad->setId(newId);
//
//	m_annotations[ad->id()] = ad;
//	ad->setParent(this);
//
//	return ad->id();
//}
//
//CAnnotation * CAnimation::removeAnnotation( int id )
//{
//	CBaseObject *an = getSomethingWithId( id );
//	
//	if (an == nullptr) return nullptr;
//	if (an->category() != CBaseObject::Category::ANNOTATION) return nullptr;
//
//	CBaseObject* parent = an->getParent();
//
//	if (parent == this)
//	{
//		m_annotations.erase(id);
//	}
//	else
//	{
//		((CAnnotation*)parent)->removeAnnotation(id);
//	}
//
//	return (CAnnotation*) an;
//}
//
//CAnnotation * CAnimation::annotation(int id)
//{
//	Annotations::iterator it;
//	for (it = m_annotations.begin(); it != m_annotations.end(); it++)
//	{
//		if ( it->first == id ) return it->second;
//		CAnnotation* obj = it->second->annotation(id);
//		if (obj != nullptr) return obj;
//	}
//	return nullptr;
//}

#include "FileConnector.h"

CAnimation* CAnimation::load(const QString fext, const QString path, bool synchronous)
{
	// CParser* parser = CFileConnector::getLoadParser(fext);

	// if (nullptr != parser)
	// {
	// 	CAnimation* obj = parser->load(path, synchronous);

	// 	if (!parser->inPlugin()) delete parser;

	// 	return obj;
	// }

	// return nullptr;
	return nullptr;
}

CAnimation* CAnimation::load(const QString path, bool synchronous)
{
	// CParser* parser = CFileConnector::getLoadParser( path );

	// if (nullptr != parser)
	// {
	// 	CAnimation* obj = parser->load( path, synchronous );

	// 	if (!parser->inPlugin()) delete parser;

	// 	return obj;
	// }

	return nullptr;
}

bool CAnimation::save(const char* path)
{
	return save( QString(path).toStdWString() );
}

bool CAnimation::save(const QString path)
{
	// CParser* parser = CFileConnector::getSaveParser( path );

	// if (NULL != parser)
	// {
	// 	//parser->set( UI::getNativePath(path), this );
	// 	//bool result = parser->save();

	// 	bool result = parser->save(this, path );

	// 	if (!parser->inPlugin()) delete parser;

	// 	return result;
	// }

	// return CFileConnector::save( this, path );
	return false;
}

bool CAnimation::save(const std::string path)
{
	return save( QString::fromStdString(path).toStdWString() );
}

#include "FileConnector.h"

bool CAnimation::save(std::wstring path)
{
	return save( QString::fromStdWString(path) );
}


// void drawBox(CPoint3f min, CPoint3f max, bool dashed=false)
// {
// 	if (dashed)
// 	{
// 		glLineStipple(3, 0x00FF);
// 		glEnable(GL_LINE_STIPPLE);
// 	}

// 	glPushAttrib(GL_ENABLE_BIT);
// 	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

// 	glBegin(GL_QUAD_STRIP);
// 	glVertex3f(max.X(), min.Y(), min.Z());
// 	glVertex3f(max.X(), min.Y(), max.Z());

// 	glVertex3f(min.X(), min.Y(), min.Z());
// 	glVertex3f(min.X(), min.Y(), max.Z());

// 	glVertex3f(min.X(), max.Y(), min.Z());
// 	glVertex3f(min.X(), max.Y(), max.Z());

// 	glVertex3f(max.X(), max.Y(), min.Z());
// 	glVertex3f(max.X(), max.Y(), max.Z());

// 	glVertex3f(max.X(), min.Y(), min.Z());
// 	glVertex3f(max.X(), min.Y(), max.Z());
// 	glEnd();

// 	glPopAttrib();
// }

void CAnimation::renderBoundingBox()
{
	CBoundingBox::Style style;

	if (AP::WORKSPACE::getCurrentModelId() != m_Id)
		style = CBoundingBox::Style::NotSelected;
	else
		if (isLocked())
			style = CBoundingBox::Style::Locked;
		else
			style = CBoundingBox::Style::Unlocked;

	if (AP::WORKSPACE::SELECTION::isModelSelected(m_Id))
	{
		CBoundingBox::draw(style, true);
	}
	else
	{
		CBoundingBox::draw(style, false);
	}
}

//void CAnimation::renderBoundingBox()
//{
//	glPushAttrib(GL_ALL_ATTRIB_BITS);
//
//	glDisable(GL_TEXTURE_2D);
//
//	glEnable(GL_COLOR_MATERIAL);
//	glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);
//
//	glLineWidth(1.0);
//	if ( AP::WORKSPACE::getCurrentModelId() == m_Id )
//		if (isLocked())
//			glColor3f(1.0f, 0.0f, 0.0f);
//		else
//			glColor3f(0.0f, 0.5f, 0.0f);
//	else
//		glColor3f(0.2f, 0.2f, 0.2f);
//
//	drawBox(m_min, m_max);
//
//	if (AP::WORKSPACE::SELECTION::isModelSelected(m_Id))
//	{
//		glLineWidth(3.0);
//		glColor4f(0.5f, 0.5f, 0.0f, 0.5f);
//		drawBox(m_min, m_max, true);
//	}
//
//	glDisable(GL_COLOR_MATERIAL);
//
//	glPopAttrib();
//}

void CAnimation::renderAxes()
{
	glPushAttrib(GL_ALL_ATTRIB_BITS);

	glDisable(GL_TEXTURE_2D);
	glEnable(GL_COLOR_MATERIAL);
	glColorMaterial(GL_FRONT, GL_AMBIENT_AND_DIFFUSE);

	float buf[4];
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, CRGBA(_DEF_AMBIENT).fV(buf));
	glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, CRGBA(_DEF_DIFFUSE).fV(buf));
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, CRGBA(_DEF_SPECULAR).fV(buf));
	glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, CRGBA(_DEF_EMISSION).fV(buf));
	glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, _DEF_SHININESS);

	glLineWidth(2.0);

	int L = 5;

	glColor4f(1.0f, 0.0f, 1.0f, 1.0f);

	glBegin(GL_LINES);

	glColor4f(0.0f, 0.0f, 1.0f, 1.0f);
	glVertex3f(-L, 0, 0);
	glVertex3f( L, 0, 0);

	glColor4f(0.0f, 1.0f, 0.0f, 1.0f);
	glVertex3f(0, -L, 0);
	glVertex3f(0,  L, 0);

	glColor4f(1.0f, 0.0f, 0.0f, 1.0f);
	glVertex3f(0, 0, -L);
	glVertex3f(0, 0,  L);
	
	glEnd();

	//glColor3f(0.5f, 0.5f, 0.5f);
	//glEnable(GL_POINT_SMOOTH);
	//glPointSize(5);
	//glBegin(GL_POINTS);
	//glVertex3f(0, 0, 0);
	//glEnd();

	glDisable(GL_COLOR_MATERIAL);

	glPopAttrib();
}

//int CAnimation::setAnimation(int anim)
//{
//	if (bAnime = (0 != anim))
//		iAnimeDir = (anim<0) ? -1 : 1;
//
//	lastanim = GetTickCount64();
//
//	return bAnime ? iAnimeDir : 0;
//}
//
//int CAnimation::toggleAnimation()
//{
//	lastanim = GetTickCount64();
//	return (bAnime = !bAnime) ? iAnimeDir : 0;
//}
//
//int CAnimation::toggleAnimationDir()
//{
//	if (bAnime)
//		return iAnimeDir = -iAnimeDir;
//	return 0;
//}
//


//CBaseObject::Visibility CAnimation::setVisibility(CBaseObject::Visibility v)
//{
//	if (v == Visibility::HIDE_SELF)
//	{
//		return (m_visible = Visibility::HIDE_ALL);
//	}
//	return (m_visible = v);
//}
//
//CBaseObject::Visibility CAnimation::switchVisibility()
//{
//	if 	((m_visible == HIDE_ALL) || (m_visible == HIDE_SELF))
//		return m_visible = SHOW;
//	else
//		return m_visible = HIDE_ALL;
//}


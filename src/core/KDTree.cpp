	...
	if (npts) {
		for (int i = 0; i < npts; i++)
		{
				...
				&& (!ti.iscompat || (*ti.iscompat)(leaf.points[i])))
			{
				...
				push_heap(ti.knn.begin(), ti.knn.end());      // po tym najwi�kszy element trafia do knn[0]
				
				if (ti.knn.size() > ti.k)
				{
					pop_heap(ti.knn.begin(), ti.knn.end());  // po tym najwi�kszy element trafia do knn[k-1], a w knn[0] jest drugi najwi�kszy
					ti.knn.pop_back();
				}
				
				// Keep track of distance to k-th closest
				ti.closest_d = ti.knn[0].first;              // w knn[0] powinien byc najwi�kszy element
...
void KDtree::Node::find_k_closest_to_ray(KDtree::Node::Traversal_Info &ti) const
{
	...
	if (npts) {
		for (int i = 0; i < npts; i++) {
			// We don't want to find the point itself  <--- a tak w�a�ciwie, to dlaczego ????
			if (leaf.points[i] == ti.p)
				continue;

			float myd2 = dist2ray2(leaf.points[i], ti.p, ti.dir); //kwadrat odleg�o�ci punktu od promienia
			...
			if ( ( myd2 < ti.closest_d2 ) && (!ti.iscompat || (*ti.iscompat)(leaf.points[i])))
			{
				...
				ti.knn.push_back(make_pair(myd, leaf.points[i])); //dok�ada element do wektora
				push_heap(ti.knn.begin(), ti.knn.end());	// przesuwa najwi�kszy element na pocz�tek wektora
				
				if (ti.knn.size() > ti.k) {
					pop_heap(ti.knn.begin(), ti.knn.end()); // przesuwa najwi�kszy element na koniec wektora,
															// a jednocze�nie na pocz�tku pojawia sie drugi w kolejno�ci element

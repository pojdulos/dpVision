...
public:
	...
	/* tworzy map� powi�za� indeks�w wierzcho�k�w z indeksami �cianek */
	...
	/* szuka wszystkich s�siadek �cianki i podanym indeksie, s�siedztwo mo�e byc po kraw�dziach (limit=1) lub po wierzcho�kach (limit=0)*/
	void findNeighborsOfFace( INDEX_TYPE idxF, CMesh::V2Fmap &map, CMesh::Neighbors *sasiadki, int limit = 1);

	/* zwraca uporz�dkowany zbi�r (std::set) �cianek brzegowych dla ca�ej siatki */
	void findBoundaryFaces( std::set<INDEX_TYPE> &brzegowe, CMesh::V2Fmap &map );

	/* zwraca map� (std::map) �cianek brzegowych wraz z ich s�siedztwem dla ca�ej siatki */
	void findBoundaryFaces( CMesh::Neighborhood &brzegowe, CMesh::V2Fmap &map);

	/* zwraca uporz�dkowany zbi�r (std::set) �cianek brzegowych dla podanego podzbioru �cianek */
	void findBoundaryFaces( std::set<INDEX_TYPE> &brzegowe, CMesh::V2Fmap &map, std::set< INDEX_TYPE > &scianki);

	/* zwraca map� (std::map) �cianek brzegowych wraz z ich s�siedztwem dla podanego podzbioru �cianek */
	void findBoundaryFaces( CMesh::Neighborhood &brzegowe, CMesh::V2Fmap &map, std::set< INDEX_TYPE > &scianki);

	/* szuka kraw�dzi brzegowych gdy mamy juz znalezione �cianki brzegowe */
	void findBoundaryEdges( CMesh::Edges &edges, CMesh::Neighborhood sciankiBrzegowe );

	/* zwraca zbi�r (std::set) wszystkich kraw�dzi w siatce */
	void getEdges( CMesh::Edges &edges);

	/* zwraca zbi�r (std::set) wszystkich kraw�dzi dla podanego podzbioru �cianek */
	void getEdges( CMesh::Edges &edges, std::set< INDEX_TYPE > &scianki);

	/* zwraca zbi�r (std::set) wszystkich kraw�dzi brzegowych w siatce */
	void findBoundaryEdges( CMesh::Edges &edges );

	/* zwraca zbi�r (std::set) wszystkich kraw�dzi brzegowych dla podanego podzbioru �cianek */
